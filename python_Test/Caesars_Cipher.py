...
# In cryptography, a Caesar cipher is a very simple encryption techniques in which each letter in the plain text is replaced by
...
def caesar_cipher_encoder_decoder(sentence):
    key1 = {
        'a':'n', 'b':'o', 'c':'p', 'd':'q', 'e':'r', 'f':'s', 'g':'t', 'h':'u', 'i':'v', 'j':'w', 'k':'x', 'l':'y', 'm':'z',
        'n':'a', 'o':'b', 'p':'c', 'q':'d', 'r':'e', 's':'f', 't':'g', 'u':'h', 'v':'i', 'w':'j', 'x':'k', 'y':'l', 'z':'m',
        'A':'N', 'B':'O', 'C':'P', 'D':'Q', 'E':'R', 'F':'S', 'G':'T', 'H':'U', 'I':'V', 'J':'W', 'K':'X', 'L':'Y', 'M':'Z',
    ...
    for s in sentence:
        for key, value in key1.items():
            if key == s:
                new_sentence = new_sentence + value
            else:
                pass
    print(new_sentence)


caesar_cipher_encoder_decoder("Hello I am Umair")
caesar_cipher_encoder_decoder("Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!")